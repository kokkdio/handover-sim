...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path("results/trajectories/trajectory"))
    parser.add_argument("--seed", type=int, default=None, help="Random seed for repeatable runs.")
    parser.add_argument(
        "--random-robot-action",
        action="store_true",
        help="Use random robot targets instead of a fixed zero action.",
    )
    parser.add_argument(
        "--action-noise",
        type=float,
        default=0.15,
        help="Uniform noise range for --random-robot-action.",
    )
    ...
    print(f"Recording trajectory from scene index {idx}. Press Ctrl+C to stop early.", flush=True)
    for step in range(1, args.max_steps + 1):
        if args.random_robot_action:
            action = np.asarray(cfg.ENV.PANDA_INITIAL_POSITION, dtype=np.float32)
            action += rng.uniform(-args.action_noise, args.action_noise, size=action.shape)
        else:
